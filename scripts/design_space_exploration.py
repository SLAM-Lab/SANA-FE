...
with open("arch/loihi.yaml", "rb") as loihi_file:
    loihi_baseline = yaml.safe_load(loihi_file)
...
if run_experiment:
    with open("runs/dvs/dse_results.csv", "w") as dse_results:
        dse_results.write("cores,comparments,energy,latency\n")

    for cores, compartments in zip(core_counts, compartment_counts):
        mapping_file = f"{cores}c_{compartments}cx_mapping.net"
        mapping_path = os.path.join(PROJECT_DIR, "runs", "dvs", "dse_mappings",
                                    mapping_file)
        ...
        loihi_baseline["architecture"]["tile"][0]["name"] = tile_name


        design_path = os.path.join(PROJECT_DIR, "runs", "dvs", "dse_mappings",
                                f"loihi_{cores}c.yaml")
        with open(design_path, "w") as design_file:
            yaml.safe_dump(loihi_baseline, design_file)

        # Concatenate the mapping with the DVS network into a temporary network file
        open("runs/dvs/dse_mappings/dvs_gesture_32x32_mapped.net", "w")
        with open("runs/dvs/dse_mappings/dvs_gesture_32x32_mapped.net", "a") as out:
            with open("runs/dvs/dse_mappings/dvs_gesture_32x32_unmapped.net", "r") as snn:
                for line in snn:
                    out.write(line)
        ...
        results = sim.run(design_path,
                        "runs/dvs/dse_mappings/dvs_gesture_32x32_mapped.net", 128)

        with open("runs/dvs/dse_results.csv", "a") as dse_results:
            dse_results.write(f"{cores},{compartments},{results['energy']},{results['time']}\n")
        print(results)

if plot_results:
    with open("runs/dvs/dse_results.csv", "r") as dse_results:
    ...
    plt.savefig("runs/dvs/dse_latency.pdf")
    ...
    plt.tight_layout(pad=0.3)
    plt.savefig("runs/dvs/dse_energy.pdf")
    ...
    df.plot.bar(rot=30, figsize=(3.6, 2.2), color=("#ff7f0e", "#1f77b4"), secondary_y="Latencies", legend=False)
    ax = plt.gca()
    plt.xlabel("Design Configuration")
    ...
    plt.tight_layout(pad=0.3)
    plt.savefig("runs/dvs/dse_product.png")
