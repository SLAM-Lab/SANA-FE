...
void py_connect_neurons(sanafe::NeuronGroup *self, sanafe::NeuronGroup &dest_group, const std::vector<std::pair<int, int>> &src_dest_id_pairs, const pybind11::dict &vals);
...
std::vector<std::string> pylist_to_str_vec(const pybind11::iterable &value)
{
	...
	{
		value_vec_str.push_back(pyobject_to_str(
...
pylist_or_object_to_str_vec(const size_t count, const pybind11::dict &vals)
{
	...
	for (auto &v: vals)
	{
		...
		if (!pybind11::isinstance<pybind11::iterable>(v.second))
		{
			...
			std::vector<std::string> vec(count, pyobject_to_str(
...
std::map<std::string, std::string> dict_to_str_map(
	const pybind11::dict &vals)
{
	...
	for (const auto &v: vals)
	{
		...
		const std::string key = pybind11::cast<std::string>(v.first);
		const std::string value_str = pyobject_to_str(
			pybind11::cast<pybind11::object>(v.second));
		map[key] = value_str;
...
std::string pyobject_to_str(const pybind11::object &value)
{
	...
	else
	{
		throw std::invalid_argument(
			"Error: dict has unsupported type");
	}
...
PYBIND11_MODULE(sanafecpp, m)
{
	...
	pybind11::register_exception<std::invalid_argument>(m, "KernelInvalidArgument");
	...
	pybind11::class_<sanafe::Network>(m, "Network")
		...
		.def("create_neuron_group", [](sanafe::Network *self, const int neuron_count, pybind11::dict &attr)->sanafe::NeuronGroup &
			{ return self->create_neuron_group(neuron_count, dict_to_str_map(attr)); },
	...
	pybind11::class_<sanafe::NeuronGroup>(m, "NeuronGroup")
		...
		.def("set_attribute_multiple", [](sanafe::NeuronGroup *self, const std::string &attr_name, pybind11::iterable &values)
			{ return self->set_attribute_multiple(attr_name, pylist_to_str_vec(values)); })
		.def("connect_neurons", [](
	...
	pybind11::class_<sanafe::Neuron>(m, "Neuron")
		...
		.def("connect_to_neuron",
			[](sanafe::Neuron *self, sanafe::Neuron &dest,
				...
				std::optional<pybind11::dict> attr=pybind11::none())
			{
				if (!attr.has_value())
				{
					attr = pybind11::dict();
	...
	pybind11::class_<sanafe::Architecture>(m, "Architecture")
		...
		.def("load_arch_description", &sanafe::Architecture::load_arch_description)
		.def("save_arch_description", &sanafe::Architecture::save_arch_description)
		.def("create_tile", [](sanafe::Architecture *self, const std::string &name, pybind11::dict &attr)->sanafe::Tile &
			{ return self->create_tile(name, dict_to_str_map(attr)); },
			pybind11::return_value_policy::reference_internal)
		.def("create_core", [](sanafe::Architecture *self, const std::string &name, const size_t tile_id, pybind11::dict &attr)->sanafe::Core &
			{ return self->create_core(name, tile_id, dict_to_str_map(attr)); },
			pybind11::return_value_policy::reference_internal)
		.def("set_noc_attributes", [](sanafe::Architecture *self, pybind11::dict &attr)
	...
	pybind11::class_<sanafe::Core, std::shared_ptr<sanafe::Core>>(m, "Core")
		.def(pybind11::init<const std::string, const int, const int, const int>(),
			pybind11::arg("name"),
			pybind11::arg("core_id"),
			pybind11::arg("tile_id"),
			pybind11::arg("offset"))
		.def("__repr__", &sanafe::Core::info)
		.def("create_axon_in", []
			(sanafe::Core *self, const std::string &name, pybind11::dict &attr)
			{ return self->create_axon_in(name, dict_to_str_map(attr)); },
			pybind11::return_value_policy::reference_internal)
		.def("create_dendrite", [](sanafe::Core *self, const std::string &name, pybind11::dict &attr)
			{ return self->create_dendrite(name, dict_to_str_map(attr)); },
			pybind11::return_value_policy::reference_internal)
		.def("create_synapse", [](sanafe::Core *self, const std::string &name, pybind11::dict &attr)
			{ return self->create_synapse(name, dict_to_str_map(attr)); },
			pybind11::return_value_policy::reference_internal)
		.def("create_soma", [](sanafe::Core *self, const std::string &name, pybind11::dict &attr)
			{ return self->create_soma(name, dict_to_str_map(attr)); },
			pybind11::return_value_policy::reference_internal)
		.def("create_axon_out", [](sanafe::Core *self, const std::string &name, pybind11::dict &attr)
			{ return self->create_axon_out(name, dict_to_str_map(attr)); },
			pybind11::return_value_policy::reference_internal)
	...
	pybind11::class_<sanafe::Simulation>(m, "Simulation")
		.def(pybind11::init<sanafe::Architecture &, sanafe::Network &, std::string, bool, bool, bool, bool>(),
			pybind11::arg("arch"),
			pybind11::arg("net"),
		...
		.def("run", [](sanafe::Simulation *self, const long int timesteps, const long int heartbeat)
		{ return run_data_to_dict(self->run(timesteps, heartbeat)); },
