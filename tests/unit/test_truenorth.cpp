...
TEST_F(TestTrueNorthModel, SetThresholdAndUpdateFires)
{
    ...
    auto result = model.update(0UL, 1.0, 1L); // Add current
...
TEST_F(TestTrueNorthModel, LeakReducesPotential)
{
    ...
    model.update(0UL, 2.0, 1L); // initial current
    double before = model.get_potential(0);

    model.update(0UL, std::nullopt, 1L); // leak applies
...
{
    model.set_attribute_neuron(0UL, "threshold", make_attr_double(5.0));
    model.set_attribute_neuron(0UL, "reset_mode", make_attr_string("hard"));
    model.set_attribute_neuron(0UL, "reset", make_attr_double(0.0));

    model.update(0UL, 10.0, 1L);
    EXPECT_GE(model.get_potential(0UL), 0.0);
}

TEST_F(TestTrueNorthModel, RandomMaskNegativeThrows)
{
    EXPECT_THROW(
            model.set_attribute_neuron(0UL, "random_mask", make_attr_int(-1)),
...
TEST_F(TestTrueNorthModel, RandomMaskEnablesRandomizedThreshold)
{
    ...
    auto r = model.update(0UL, std::nullopt, 1L);
